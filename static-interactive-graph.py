...
legacy_graph_dataframes = {"902b pressure": pd.DataFrame()}
...
def getDataFromWebMonitorFile(filename: Optional[str]) -> pd.DataFrame:
    ...
    if (
        int_settings["WM time window size (minutes)"] != 0
        and not _webmon_cache_df.empty
    ):
        cutoff = _webmon_cache_df.index.max() - pd.Timedelta(
            minutes=int_settings["WM time window size (minutes)"]
        )
...
# This function uses regex, so it is a bit slow and should be called on only if needed
# (i.e. if the 902b pressure graph is enabled)
...
def get902bPressureData(filename: str) -> pd.DataFrame:
    ...
    regex_pattern = re.compile(
        r"\[\d{4}-\d{2}-\d{2} (\d{2}:\d{2}:\d{2})\.\d{3}\] @\d{3}ACK(\d*\.\d*);FF", re.I
    )
...
def getAllData() -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame]]:
    ...
    if any(cfg.get("enabled") for cfg in legacy_graph_settings.values()):
        ...
        pressure_df = (
            get902bPressureData(teraTerm_log_file902b)
            if teraTerm_log_file902b
            else pd.DataFrame(columns=["Time", "Pressure (mbar)"])
        )
...
def getGraph(numPlots: int) -> Any:
    ...
    fig, axs = plt.subplots(
        numRows,
        int_settings["number of columns"],
        figsize=(int_settings["fig width"], int_settings["fig height"]),
        sharex=True,
    )
...
def updateGraph(
        ...
        if graph_settings[subplot]["hasData"] and graph_settings[subplot]["enabled"]:
            for col in graph_settings[subplot]["lines"]:
                label = (
                    col
                    + " ("
                    + webMonitor_df[col].iloc[-1].astype(str)
                    + graph_settings[subplot]["unit"]
                    + ")"
                )
                if numCols == 1:
            ...
            if numCols == 1:
                ...
                axs[curr_row].yaxis.set_major_locator(
                    ticker.MaxNLocator(nbins=int_settings["fig y ticks"])
                )
                curr_row += 1
            else:
                ...
                axs[curr_row, curr_col].yaxis.set_major_locator(
                    ticker.MaxNLocator(nbins=int_settings["fig y ticks"])
                )
    ...
    for entry in legacy_graph_settings:
        if (
            legacy_graph_settings[entry]["hasData"]
            and legacy_graph_settings[entry]["enabled"]
        ):
            for col in legacy_graph_settings[entry]["lines"]:
                ...
                label = (
                    col
                    + " ("
                    + dataframe[col].iloc[-1].astype(str)
                    + legacy_graph_settings[entry]["unit"]
                    + ")"
                )
                ...
                if numCols == 1:
                    ...
                    axs[curr_row, curr_col].plot(
                        dataframe["Time"], dataframe[col], label=label
                    )
    ...
    if numCols == 1:
        ...
        for col in range(numCols):
            axs[numRows - 1, col].xaxis.set_major_locator(
                ticker.MaxNLocator(nbins=int(40 / numCols))
            )
            axs[numRows - 1, col].xaxis.set_major_formatter(
